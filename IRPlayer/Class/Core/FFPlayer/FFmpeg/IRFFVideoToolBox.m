...
typedef NS_ENUM(NSUInteger, IRFFVideoToolBoxErrorCode) {
    IRFFVideoToolBoxErrorCodeExtradataSize,
    IRFFVideoToolBoxErrorCodeExtradataData,
    IRFFVideoToolBoxErrorCodeCreateFormatDescription,
    IRFFVideoToolBoxErrorCodeCreateSession,
    IRFFVideoToolBoxErrorCodeNotH264,
};

@interface IRFFVideoToolBox ()
...
@implementation IRFFVideoToolBox
...
- (NSError *)setupVTSession
{
    ...
    if (codec_id == AV_CODEC_ID_H264) {
        if (extradata_size < 7 || extradata == NULL) {
            error = [NSError errorWithDomain:@"extradata error" code:IRFFVideoToolBoxErrorCodeExtradataSize userInfo:nil];
            return error;
        }
        ...
        if (extradata[0] == 1) {
            ...
            if (self->_format_description == NULL) {
                error = [NSError errorWithDomain:@"create format description error" code:IRFFVideoToolBoxErrorCodeCreateFormatDescription userInfo:nil];
                return error;
            }
...
#elif IRPLATFORM_TARGET_OS_IPHONE
            ...
            if (status != noErr) {
                error = [NSError errorWithDomain:@"create session error" code:IRFFVideoToolBoxErrorCodeCreateSession userInfo:nil];
                return error;
            }
            CFRelease(destinationPixelBufferAttributes);
            return nil;
        } else {
            error = [NSError errorWithDomain:@"deal extradata error" code:IRFFVideoToolBoxErrorCodeExtradataData userInfo:nil];
            return error;
        }
    } else {
        error = [NSError errorWithDomain:@"not h264 error" code:IRFFVideoToolBoxErrorCodeNotH264 userInfo:nil];
        return error;
    }
...
{
    [self flush];
    NSLog(@"IRFFVideoToolBox release");
}
...
static void outputCallback(void * decompressionOutputRefCon, void * sourceFrameRefCon, OSStatus status, VTDecodeInfoFlags infoFlags, CVImageBufferRef imageBuffer, CMTime presentationTimeStamp, CMTime presentationDuration)
{
    @autoreleasepool
    {
        IRFFVideoToolBox * videoToolBox = (__bridge IRFFVideoToolBox *)decompressionOutputRefCon;
